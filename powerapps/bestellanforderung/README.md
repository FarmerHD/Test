...
1. „Moderne Steuerelemente und Designs“ aktiviert lassen (Einstellungen > Updates).
2. SharePoint-Listen `Antraege`, `Kommentare`, `Rollen` mit genau den Spaltennamen oben anlegen.
...
6. Datei-Upload:
   ...
   - In `btnAbsenden.OnSelect` im `Patch` nach `Defaults(Antraege),` die Zeile
     `frmAnhaenge.Updates,` ergänzen und am Ende `ResetForm(frmAnhaenge);` anhängen.
7. Fachbereiche: Die Werte im Dropdown `ddFachbereich` (Items) müssen mit den
   Auswahlwerten der Spalte `Fachbereich` übereinstimmen.

Kontrolle nach dem Einfügen: In `Card_Antrag` stehen 19 Elemente, in
`Container_Content_A` zusätzlich drei Nav-Buttons.
